...
class ContentCreationTeam:
    ...
    def create_content(self, content_brief: str, content_type: str = "general") -> Dict[str, Any]:
        ...
        try:
            ...
            brand_analysis = self._get_brand_strategy_analysis(content_brief, content_type)
            
            # Step 2: Enhance brief with context and brand strategy
            enhanced_brief = self._enhance_brief_with_context(content_brief, content_type, brand_analysis)
            ...
            self.project_manager.agent.initiate_chat(
                manager,
                message=enhanced_brief
            )
        ...
        results = self._extract_and_save_results(enhanced_brief, content_type,groupchat, None)
    ...
    def _get_brand_strategy_analysis(self, brief: str, content_type: str) -> Dict[str, Any]:
        ...
        print(f"    Brand strategy: {brand_analysis['positioning_strategy']['messaging_focus']}")
        print(f"    Brand archetype: {brand_analysis['brand_archetype']['description']}")
    ...
    def _extract_industry_from_brief(self, brief: str) -> str:
        ...
        industry_keywords = {
            'technology': ['tech', 'software', 'digital', 'app', 'platform', 'ai', 'ml'],
            'healthcare': ['health', 'medical', 'doctor', 'hospital', 'medicine', 'wellness'],
            'education': ['education', 'learning', 'school', 'course', 'training', 'skill'],
            'finance': ['finance', 'banking', 'investment', 'money', 'loan', 'insurance'],
    ...
    def _extract_audience_from_brief(self, brief: str) -> str:
        ...
        brief_lower = brief.lower()
        audience_keywords = {
            'young_professionals': ['professional', 'career', 'young', 'graduate', 'employee'],
            'families_with_children': ['family', 'children', 'kids', 'parent', 'mother', 'father'],
    ...
    def _enhance_brief_with_context(self, brief: str, content_type: str, brand_analysis: Dict[str, Any]) -> str:
    ...
    def _extract_and_save_results(self, brief: str, content_type: str, groupchat=None, final_messages=None) -> Dict[str, Any]:
        ...
        results = {
            ...
            "agents_used": ["ProjectManager", "ContentWriter", "ContentEditor", "SEOSpecialist", "BrandStrategist"],
            "status": "completed",
            "output_files": [],
            "conversation_stats": {},
            "final_content": {}
        ...
        try:
            ...
            if messages:
                results["conversation_stats"] = {
                    ...
                    "agents_participated": len(set(msg.get('name', 'Unknown') for msg in messages)),
                    "message_breakdown": {}
                }
                ...
                for msg in messages:
                    ...
                    if agent_name not in results["conversation_stats"]["message_breakdown"]:
                        results["conversation_stats"]["message_breakdown"][agent_name] = 0
            ...
            if final_content.get("final_campaign"):
                ...
                with open(campaign_file, "w", encoding="utf-8") as f:
                    ...
                    f.write("=" * 80 + "\n")
                    f.write(f"Content Type: {content_type}\n")
                    f.write(f"Generated By: {final_content.get('generated_by', 'Multiple Agents')}\n")
                    f.write(f"Word Count: {len(final_content['final_campaign'].split())}\n")
                    f.write(f"Character Count: {len(final_content['final_campaign'])}\n")
            ...
            if final_content.get("agent_contributions"):
                contributions_file = self.work_dir / f"agent_contributions_{timestamp}.json"
                with open(contributions_file, "w", encoding="utf-8") as f:
                    json.dump(final_content["agent_contributions"], f, indent=2, ensure_ascii=False)
            ...
            print(f"💬 Conversation: {results['conversation_stats'].get('total_messages', 0)} messages")
            print(f"🤖 Agents participated: {results['conversation_stats'].get('agents_participated', 0)}")
    ...
    def _extract_final_campaign_content(self, messages: list) -> Dict[str, Any]:
            ...
            try:
                ...
                substantial_messages = [msg for msg in messages if len(msg.get("content", "")) > 100]
    ...
    def _load_regional_config(self) -> Dict[str, Any]:
        ...
        return {
            "target_regions": ["mumbai", "delhi", "bangalore", "pune", "hyderabad", "chennai"],
            "languages": {
                "primary": "english",
                "secondary": "hindi",
    ...
    def _setup_llm_config(self) -> Dict[str, Any]:
        ...
        if provider == "azure":
            ...
            llm_config = {
                "config_list": [{
                    "model": os.getenv("AZURE_OPENAI_DEPLOYMENT_MODEL", "gpt-35-turbo"),
                    "api_key": api_key,
                    "api_type": "azure",
    ...
    def create_blog_article(self, topic: str, target_audience: str = "Indian businesses", 
                          word_count: int = 1500) -> Dict[str, Any]:
    ...
    def create_email_campaign(self, campaign_objective: str, audience_segment: str) -> Dict[str, Any]:
    ...
    def analyze_competitor_content(self, competitor_info: str, analysis_focus: str) -> Dict[str, Any]:
...
def create_content_team_with_openai(api_key: str, model: str = "gpt-4") -> ContentCreationTeam:
        """
...
def create_content_team_with_azure(api_key: str, endpoint: str, model: str,
                                 api_version: str = "2024-02-15-preview") -> ContentCreationTeam:
        """
