...
class BaseAgent(ABC):
    ...
    def _create_agent(self):
        ...
        if self.agent_type.lower() == "userproxy":
            return UserProxyAgent(
                ...
                max_consecutive_auto_reply=self.agent_config.get(
                    "max_consecutive_auto_reply", 3
                ),
                is_termination_msg=self._is_termination_msg,
                code_execution_config=self.agent_config.get(
    ...
    def get_agent_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "llm_provider": self.llm_config.get("config_list", [{}])[0].get(
                "api_type", "unknown"
            ),
            "regional_focus": self.regional_config.get("target_regions", [])[
                :5
            ],  # Top 5 regions
        }
    ...
    def update_performance_metrics(self, response_time: float, success: bool):
        ...
        if self.conversation_count == 1:
            ...
            self.success_rate = (
                self.success_rate * (self.conversation_count - 1)
        ...
        if self.conversation_count == 1:
            ...
            self.average_response_time = (
                self.average_response_time * (self.conversation_count - 1)
                + response_time
            ) / self.conversation_count

    def save_conversation_log(
        self, conversation_data: Dict[str, Any], output_dir: Path
    ):
    ...
    def validate_content_quality(self, content: str) -> Dict[str, Any]:
        ...
        # Overall score
        scores = [
            score for score in quality_check.values() if isinstance(score, (int, float))
        ]
    ...
    def _check_cultural_sensitivity(self, content: str) -> bool:
        ...
        sensitive_terms = ["backward", "primitive", "third world", "developing country"]
