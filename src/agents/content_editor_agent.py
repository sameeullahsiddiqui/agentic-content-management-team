...
class ContentEditorAgent(BaseAgent):
    ...
    def edit_content(self, content: str, content_type: str, target_audience: Dict[str, Any]) -> Dict[str, Any]:
        ...
        market_enhanced = self._enhance_indian_market_relevance(readability_optimized["content"])
        ...
        editing_result["quality_scores"] = self._calculate_quality_scores(final_content["content"])
        editing_result["recommendations"] = self._generate_recommendations(editing_result)
    ...
    def _get_editing_standards(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "grammar_and_language": {
                "grammar_accuracy": {"target": 99, "critical_errors": ["subject_verb_disagreement", "tense_inconsistency"]},
                "spelling_accuracy": {"target": 100, "dictionary": "indian_english"},
                "punctuation": {"target": 98, "style": "oxford_comma_optional"},
                "sentence_structure": {"avg_words_per_sentence": 15, "max_words_per_sentence": 25}
            },
            "readability": {
    ...
    def _get_indian_style_guide(self) -> Dict[str, Any]:
        ...
        return {
            "spelling_conventions": {
                "british_vs_american": {
                    "prefer_british": ["realise", "colour", "centre", "honour", "favour"],
                    "commonly_accepted_american": ["organize", "recognize", "mobile", "online"]
                },
                "indian_specific": {
            ...
            "hindi_integration": {
                "common_accepted_words": [
                    "chai", "namaste", "ji", "sahab", "madam", "beta", "didi", "bhai",
                    "gharelu", "desi", "videshi", "swadeshi", "jugaad", "aam aadmi"
                ],
                "business_terms": [
                    "vyavasaya", "kaam", "safalta", "unnati", "pragati", "vikas"
                ],
                "usage_rules": {
                    "italicize": False,
    ...
    def _analyze_content_structure(self, content: str) -> Dict[str, Any]:
        ...
        if analysis["avg_paragraph_length"] > 100:
            analysis["structure_issues"].append("Paragraphs too long for mobile reading")
        
        if analysis["total_words"] > 200 and not analysis["has_headers"]:
            analysis["structure_issues"].append("Long content needs headers for better navigation")
    ...
    def _edit_grammar_and_language(self, content: str) -> Dict[str, Any]:
        ...
        for pattern, replacement in grammar_fixes.items():
            if re.search(pattern, edited_content, re.IGNORECASE):
                edited_content = re.sub(pattern, replacement, edited_content, flags=re.IGNORECASE)
                changes_made.append(f"Fixed common error: '{pattern}' → '{replacement}'")
    ...
    def _review_cultural_sensitivity(self, content: str) -> Dict[str, Any]:
        ...
        sensitive_terms = [
            "backward", "primitive", "uneducated", "illiterate", "poor people",
            "rich people", "typical indian", "all indians", "village people"
        ]
        
        for term in sensitive_terms:
            if term in content_lower:
                assessment["sensitivity_score"] -= 10
                assessment["issues_found"].append(f"Potentially insensitive term: '{term}'")
                assessment["suggestions"].append(f"Replace '{term}' with more respectful language")
        ...
        if len(regions_mentioned) >= 2:
            ...
            assessment["suggestions"].append("Consider adding examples from other Indian regions for better balance")
        ...
        for i, term in enumerate(exclusive_terms):
            if term in content_lower:
                ...
                assessment["suggestions"].append(f"Replace '{term}' with '{inclusive_alternatives[i]}'")
    ...
    def _optimize_readability(self, content: str) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "total_sentences": len(sentences),
            "estimated_grade_level": min(12, max(1, (len(words) / len(sentences)) - 5)) if sentences else 8,
    ...
    def _enhance_indian_market_relevance(self, content: str) -> Dict[str, Any]:
        ...
        if re.search(currency_pattern, enhanced_content):
            enhanced_content = re.sub(currency_pattern, r'₹ \1 (equivalent)', enhanced_content)
            enhancements.append("Added Indian currency context to international examples")
    ...
    def _optimize_for_mobile(self, content: str) -> Dict[str, Any]:
        ...
        list_indicators = ['benefits include', 'features are', 'we offer', 'services include']
    ...
    def _has_indian_context(self, content: str) -> bool:
        ...
        indian_indicators = [
            ...
            "rupee", "₹", "lakh", "crore", "diwali", "holi", "bollywood", 
            "cricket", "chai", "namaste"
        ]
    ...
    def _generate_recommendations(self, editing_result: Dict[str, Any]) -> List[str]:
        ...
        if scores["readability_score"] < 80:
            recommendations.append("Consider shortening sentences and using simpler vocabulary for better readability")
        
        if scores["cultural_relevance"] < 70:
            recommendations.append("Add more Indian examples, statistics, or cultural references to increase local relevance")
        
        if scores["mobile_optimization"] < 80:
            recommendations.append("Break long paragraphs into shorter chunks for better mobile reading experience")
        
        if scores["engagement_potential"] < 60:
            recommendations.append("Include more questions, direct address to readers, or interactive elements")
        
        # Content-specific recommendations
        content = editing_result["edited_content"]
        
        if len(content.split()) > 500 and not re.search(r'^\s*[•\-\*]', content, re.MULTILINE):
            recommendations.append("Consider adding bullet points or numbered lists for easier scanning")
        
        if "call" in content.lower() or "contact" in content.lower():
            recommendations.append("Ensure contact methods include WhatsApp and local phone numbers for Indian market")
        
        if "price" in content.lower() or "cost" in content.lower():
            recommendations.append("Include Indian currency formatting and consider mentioning payment options like UPI")
    ...
    def _perform_final_quality_checks(self, content: str) -> Dict[str, bool]:
        ...
        checks = {
            ...
            "mobile_friendly_paragraphs": all(len(p.split()) <= 100 for p in content.split('\n\n') if p.strip()),
            "includes_call_to_action": bool(re.search(r'(contact|call|visit|order|buy|click|share|comment)', content, re.IGNORECASE)),
            "culturally_appropriate": not any(term in content.lower() for term in ["backward", "primitive", "illiterate"])
        }
